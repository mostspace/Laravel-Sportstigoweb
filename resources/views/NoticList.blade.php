...
<div class="dash wrapper">
  ...
  <div class="main-section pt-4 pb-4">
	...
	@forelse( $noticdetails as $userdetailsvalue )
        ...
        <td>
          <div class="action-btns">
            ...
            <a href="{{ route('noticedit', $userdetailsvalue->notice_id ) }}" class="edit-btn">
           ...
           <!--<a href="{{ route('noticdelete', $userdetailsvalue->notice_id ) }}" class="delete-btn">!-->
           <a href="javascript:void(0)" id="del{{$userdetailsvalue->notice_id}}" onclick="deleteuserfun({{$userdetailsvalue->notice_id}})" data-userid="" class="delete-btn">
              ...
              <button class="btn btn-danger m-1" style="display:none" id="deletebtn_{{ $userdetailsvalue->notice_id }}">Delete User</button>
           

          </div>
        </td>
        <td>{{date('d-m-Y', strtotime($userdetailsvalue->created_at));}}</td>
        <td>{{$userdetailsvalue->title}}</td>
        <td>{{$userdetailsvalue->message}}</td>
        <td>{{$userdetailsvalue->messagesendto}}</td>
        <td>{{$userdetailsvalue->createdby}}</td>
...
<script>
  ...
  var txtval = document.getElementById("txtserach").value; 
	...
	if (txtval == null ||
                ...
                txtval.length == 0) {
                txtval = 'Search';
				var url = "{{route('sortingnoticlist',['',''])}}" +"/"+txtval+"/"+sortval ;
				window.location.href=url;
