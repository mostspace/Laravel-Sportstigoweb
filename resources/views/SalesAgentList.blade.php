	...
	@forelse( $userdetails as $userdetailsvalue ) 
        ...
        <td>
          <div class="action-btns">
            ...
            <a href="{{ route('salesagentedit', $userdetailsvalue->id ) }}" class="edit-btn">
           ...
           <!--<a href="{{ route('salesagentdelete', $userdetailsvalue->id ) }}" class="delete-btn">!-->
		...
		<td>{{$userdetailsvalue['original_password']}}</td>
...
<script>
  ...
  function deleteuserfun(btn){
	...
	var txt;
	  ...
	  {
		var url = "{{ route('salesagentdelete', '') }}"+"/"+btn ;
  ...
  var txtval = document.getElementById("txtserach").value; 
	...
	if (txtval == null ||
                ...
                txtval.length == 0) {
                txtval = 'Search';
				var url = "{{route('sortingsalesagent',['',''])}}" +"/"+txtval+"/"+sortval ;
				window.location.href=url;
