...
<div class="dash wrapper">
  ...
  <div class="main-section pt-4 pb-4">
	...
	?>
	 ...
	 @csrf
      ...
      <div class="d-flex justify-content-end ">
        ...
        <!--  </div>-->
        <!--</div>-->
        <div><a href="{{route('featuredVendorList')}}"><button type="button" class="gray-small w-64 mr-2" style="width:200px"> Featured Vendor</button></a></div>
    ...
    <table class="table rs-table refferal-users text-center">
	...
	@forelse( $vendorlistdetails as $userdetailsvalue ) 
        ...
        <td>
          <div class="action-btns">
            ...
            <a href="{{ route('editvendor', $userdetailsvalue->vendor_id)}}" class="edit-btn">
           ...
           <!--<a href="{{ route('deletevendor', $userdetailsvalue->vendor_id)}}" class="delete-btn">!-->
           <a href="javascript:void(0)" id="del{{$userdetailsvalue->vendor_id}}" onclick="deleteuserfun({{$userdetailsvalue->vendor_id}})" class="delete-btn">
        ...
        <!--<td>{{$userdetailsvalue->address}}-->
        <!--<td>{{$userdetailsvalue->phonecode}} {{$userdetailsvalue->whatsapp}}-->
		
		
		 <?php 
		$s = $userdetailsvalue->status; 
		if($s==1)
		{
			$status = 'Approved';
			echo '<td><span class="total-earned">'.$status.'</span></td>';
		...
		{
			$status = 'Pending';
			echo '<td><span class="total-rejected">'.$status.'</span></td>';
		}		
        ...
        <td>{{$userdetailsvalue->businessname}}<!--<span class="user-box"><img src="<?php echo asset('assets/img/user.jpg'); ?>" class="user-img ml-2"></span>!--></td>
		<td>{{$userdetailsvalue->email}}

    </td>
    <td><input class="ml-4 check" value={{$userdetailsvalue->vendor_id}} type="checkbox" @if(isset($userdetailsvalue->position) && $userdetailsvalue->position != NULL ) checked @endif></td>
 ...
 /* function deleteuserfun(btn){
  ...
  $(document).ready(function() {
    $('.check').bind('change', function(e) {
      ...
      <?php if(Session::get('getsessionrole') != 1) echo "return;"?>

      if ($(this).is(':checked')) {
        var value = $(this).val();
        $.ajax({
          type: "POST",
          url: "vendorlist/featuredVendorList/add/" + value,
        ...
        $.ajax({
          type: "POST",
          url: "vendorlist/featuredVendorList/delete/" + value,
          headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}' },      
          success: function(response) {
            console.log(response);
          }
