	...
	@forelse( $userdetails as $userdetailsvalue ) 
        ...
        <td>
          <div class="action-btns">
		       <a href="{{ route('usersview', $userdetailsvalue->id ) }}" class="view-btn">
            ...
            <!--<a href="{{ route('usersedit', $userdetailsvalue->id ) }}" class="edit-btn">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
           ...
           <!--<a href="{{ route('usersdelete', $userdetailsvalue->id ) }}" class="delete-btn">!-->
           <a href="javascript:void(0)" id="del{{$userdetailsvalue->id}}" onclick="deleteuserfun({{$userdetailsvalue->id}})"  class="delete-btn">
              ...
              <button class="btn btn-danger m-1" style="display:none" id="deletebtn_{{$userdetailsvalue->id}}">Delete User</button>
        ...
        <td>{{$userdetailsvalue->mobile}}</td>
		    <td>{{$userdetailsvalue->totalbook}}
        
		<!--<span class="user-box"><img src="<?php echo asset('assets/img/user.jpg'); ?>" class="user-img ml-2"></span>!--></td>
      </tr>
 ...
 function deleteuserfun1(btn){
  ...
  var txtval = document.getElementById("txtserach").value; 
	...
	if (txtval == null ||
                ...
                txtval.length == 0) {
                txtval = 'Search';
				var url = "{{route('sortinguserlist',['',''])}}" +"/"+txtval+"/"+sortval ;
				window.location.href=url;
