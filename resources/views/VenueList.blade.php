...
<div class="dash wrapper">
  ...
  <div class="main-section pt-4 pb-4">
	 ...
	 @csrf
      ...
      <div class="d-flex justify-content-end ">
       ...
       <!-- <option value="ASC">A-Z</option>!-->
        ...
        <div align="right"><a href="{{ route('venuecreate') }}"><button type="button" class="gray-small mr-2" > BACK</button></a></div>
      </div>
      <?php 
        // echo '<pre>';
        // print_r($userdetails);
    ...
    <table class="table refferal-users w-100 text-center">
	...
	@forelse( $venuedetails as $userdetailsvalue ) 
        ...
        <td>
          <div class="action-btns">
            ...
            <a href="{{ route('venueedit', $userdetailsvalue->id ) }}" class="edit-btn">
           ...
           <a href="{{ route('venuedelete', $userdetailsvalue->id ) }}" class="delete-btn">
		...
		<td>{{$userdetailsvalue['courtdesc']}}</td>
		<td>{{$userdetailsvalue['courtprice']}}</td>
		<td>{{$userdetailsvalue['normalprice']}}</td>
      </tr>
...
<script>
  ...
  var txtval = document.getElementById("txtserach").value; 
	...
	if (txtval == null ||
                ...
                txtval.length == 0) {
                txtval = 'Search';
				var url = "{{route('sortingvenuelist',['',''])}}" +"/"+txtval+"/"+sortval ;
