      ...
      <div class="container">
        ...
        <table class="s-report table table-striped" style="width:100%;">
          ...
          <tbody>
           ...
           @if(count($withdrawaldetails) > 0)
           @foreach($withdrawaldetails as $withdrawaldtls)
           
           <tr>
              <th scope="row">{{\Carbon\Carbon::parse($withdrawaldtls->created_at)->format('d.m.Y')}}</th>
              <td>{{$withdrawaldtls->name}}</td>
              <td>RM {{number_format($withdrawaldtls->withdrawal_amount,2)}}</td>
              <td>RM {{number_format($withdrawaldtls->balance,2)}}</td>
            </tr>
          @endforeach
          @else
            <tr>
          ...
          <!-- @if(count($withdrawaldetails) > 0)
           @foreach($withdrawaldetails as $withdrawaldtls)
            @if($withdrawaldtls->reportflag== 'Y')
            <tr>
              <td colspan="7"></td>
              <td>Withdrawal Request
                  Date: {{\Carbon\Carbon::parse($withdrawaldtls->created_at)->format('d.m.Y')}}
                  Time: {{\Carbon\Carbon::parse($withdrawaldtls->created_at)->format('h.iA')}}
                  Withdrawal Amount: RM {{  number_format($withdrawaldtls->withdrawal_amount,2)}}
                  Balance: RM {{  number_format($withdrawaldtls->balance,2)}}</td>
            </tr>
            @endif 
          ...
          @else
          <tr>
              <td colspan="9" align="center"><b>No Witdrawal Request Found</b></td>            
         </tr>
         @endif!-->
...
function filterreport()
{
  ...
  var url = "{{route('vendorwithdrawreportfilter',['',''])}}" +"/"+fromdate+"/"+todate ;
