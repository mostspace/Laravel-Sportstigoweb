<?php
    ...
    class apicontroller extends Controller
    {
    		...
    		public function changepassword(Request $request)
    		{
    			...
    			if($request->password!=$request->cfpassword)  
                 ...
                 else
                 {
    				...
    				$result = DB::table('users')->where('mobile','+'.$request->mobile)->update($userdetails);
        ...
        public function saveuser(Request $request){
				...
				$checkuserregister =  DB::table('users')->where('mobile',$request->mobile)->orwhere('email',$request->email)->get();
    	...
    	public function updateuserprofile(Request $request)
        {
    		...
    		$result =  	DB::table('users')
    								->select('id','name','aboutme','interested','level')
    								->where('id',$userid)->get();
    	...
    	public function getvoucherdetails(Request $request)
    	{
    	  ...
    	  public function getallvoucherdetails(Request $request)
    	  {
    		...
    		$result	= 	DB::table('booking_with_vendors')
    					...
    					->select('booking_with_vendors.*','vendors.*')
    			...
    			if($getbookingstatus==1)
    			{
						...
						else
						{
																...
																if($hour>=5)
																{
																	...
																	$data['status'] = '0';
																	$data['message'] = 'non-refundable booking due to Cancellation Policy:5 Hours';
																	//$data['result'] = array();
      ...
      public function instructorhireme(Request $request)
      {
       ...
       public function bookingorderbyuser(Request $request)
       {
    	...
    	public function getinstructordetailsbyid(Request $request){
			...
			$result =   DB::table('instructor')
						->join('users','users.id','instructor.instructor_id')
						->select('instructor.*','users.name','users.mobile','users.email','users.role','users.user_type','users.userprofilepic1','users.aboutme','users.image1','users.image2','users.image3','users.image4','users.image5','users.image6','users.level','users.interested','users.instructorrate','users.referral_code','users.referral_user',
	   ...
	   public function membershipsbyusers(Request $request){
		...
		else
		{
			...
			$checkexistmembership	 =		DB::table('memberships')
									...
									->select('membershipsbyusers.id','users.name as name','vendors.businessname','vendors.address','membershipsbyusers.fromdate','membershipsbyusers.todate','memberships.image','membershipsbyusers.qrcodeimage','memberships.package_name',
									'memberships.package_desc1','memberships.package_desc2','memberships.package_desc3','memberships.package_desc4','memberships.package_desc5','memberships.package_duration','memberships.package_price','memberships.package_discount_price','memberships.status','membershipsbyusers.created_at','membershipsbyusers.paystatus','membershipsbyusers.paystatus_id','membershipsbyusers.transaction_id','membershipsbyusers.msg','membershipsbyusers.hash')
									->get();


			$data['status'] = '0';
			$data['message'] = 'Already exists membership for the user.';
			$data['result'] = $checkexistmembership;
			echo json_encode($data);
		}
	...
	public function membershipsrenewal(Request $request)
	{
		...
		{			
											...
											$package_price = $membershipsrenewaldetails->package_discount_price;
	...
	public function gethomepagedtls(Request $request)
				 ...
				 //->orWhere('instructor.user_name', 'LIKE', "%{$searchTerm}%")
					...
					$usersdetails =  DB::table('users')->Where('users.id',$userid)->select('users.id','users.name','users.mobile','users.userprofilepic1','users.wallet_amount','users.refferalcashback')->get();
								...
								$instructordetails	= 	DB::table('instructor')
														...
														->join('categories','categories.id','instructor.sportcategory')
														->join('vendors','vendors.businessname','instructor.sportcenter')
														->select('instructor.instructor_id as instructor_id','instructor.user_name as instructorname','users.userprofilepic1 as userprofilepic1','instructor.sportcenter as sportcenter','vendors.address as location','categories.name as sportcategory','users.instructorrate as instructorrate')
														->get();
		  ...
		  public function contactus(Request $request)
		  {
									...
									$to = 'jitendra.pxelperfect@gmail.com';
                                    ...
                                    $headers .= 'From: <pxelperfect@hotmail.com>' . "\r\n";
