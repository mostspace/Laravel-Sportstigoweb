...
class ReportsController extends Controller
{
	...
	public function vendorreport(Request $request)
    {
		...
		if($getsessioinid)
		{
			...
			if($getsessionrole==1)
			{
			  //Admin Level
                    ...
                    $bookingtransactiondtlsTotalPrice	= DB::table('booking_with_vendors')
										->join('users','users.id','booking_with_vendors.vendor_id')
										->join('booking_with_vendorsdetails','booking_with_vendorsdetails.bookingno','booking_with_vendors.bookingno')
										//->where('paystatus','Y')
										//->orwhere('paystatus','R')
		...
		exit;*/
            ...
            $total_price = 0;
            foreach ($bookingtransactiondtlsTotalPrice as $bookingtransactiondtl) {
                $total_price = $total_price + $bookingtransactiondtl->price;
            }

            // dd($total_price);


			return view('ReportVendor',compact('total_price','pagetitle','pageheading','totalAmountinAccount','todaysalesviacounter','SporstigoCommision','SalesagentCommision','netprofit','todaytotalsales','todaysalesviamobile','bookingtransactiondtls'));
	...
	public function salesreportbyvendor(Request $request,$fromdate,$todate)
	{
		...
		if($getsessioinid)
		{
			...
			return view('ReportVendor',compact('pagetitle','pageheading','totalAmountinAccount','todaysalesviacounter','SporstigoCommision','SalesagentCommision','netprofit','todaytotalsales','todaysalesviamobile','bookingtransactiondtls','total_price'));
